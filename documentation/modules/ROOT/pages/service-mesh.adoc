...
OpenShift Service Mesh automatically injects the sidecar into the Pod according to a specific label in the Deployment.
This is what *Kiali* has just added for you, lets check how that has changed the *Deployment*.

In the {OPENSHIFT_CONSOLE_URL}[OpenShift Web Console^,role='params-link'], `*click on the 'D inventory-coolstore' bubble -> 'D inventory-coolstore'*`
...
In the Container section, we should have 2 running containers: **One for the application and one for the istio-proxy sidecar.**
...
There are two parts to deploying a Gateway, firstly we create an application deployment for the ingress gateway and run this in our cn-project{USER_ID} project. 

In the {OPENSHIFT_CONSOLE_URL}[OpenShift Web Console^,role='params-link'], in the header bar next to (?)
`*click on '(+)' to 'Import YAML'*`.
...
`*Click on 'create'*`. Your ingress gateway application (Deployment, Service and Route) are now created.


The next step is to tell Istio about this Gateway option by configuring the the Gateway object and the VirtualService which describes the network 
...
`*click on http://istio-ingressgateway-cn-project{USER_ID}.{APPS_HOSTNAME_SUFFIX}/api/products[http://istio-ingressgateway-cn-project{USER_ID}.{APPS_HOSTNAME_SUFFIX}/api/products^,role='params-link']*`
...
|\http://istio-ingressgateway-cn-project{USER_ID}.{APPS_HOSTNAME_SUFFIX}
|===
FIXME
...
[TIP]
====
Use the `*show/hide>>*` control to close the side bar and make your topology chart bigger.
====
...
You can see that the traffic between the two version of the **Catalog** is shared as defined in the VirtualService 
...
Then, `*change the configuration in the YAML editor as follows*`
...
So you can see the power of Service Mesh to externally manage the traffic flow between your application components for for 
