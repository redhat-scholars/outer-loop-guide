...
OpenShift Service Mesh automatically injects the sidecar into the Pod by specifying a specific annotation in the DeploymentConfig.
...
Then, `*click on the 'YAML' tab and add __sidecar.istio.io/inject: 'true'__ under the 'spec.template.metadata.annotations' tag*`
...
The current pod will be terminated and a new one will be deployed with 2/2 containers in the **Ready** column as follows
...
`*we should find the 2 containers as well*`, one for the application, the other the istio-proxy sidecar.
