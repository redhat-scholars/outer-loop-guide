...
`*select your 'cn-project{USER_ID}'*` and `*click on 'Workloads' menu and  'Config Maps'*`. You should find the resource you just 
...
In the {OPENSHIFT_CONSOLE_URL}[OpenShift Web Console^,role='params-link'], 
`*select your 'cn-project{USER_ID}'*` and `*click on the 'Workloads' menu and 'Config Maps' -> 'CM inventory' -> 'YAML'*`.
