...
`*click on 'Workloads' menu, then 'Topology' then select your 'cn-project{USER_ID}'*` from the drop down.

image::openshift-staging-project.png[OpenShift - Staging Project, 800]
...
image::openshift-create-pipeline.png[OpenShift - Create Pipeline, 800]
...
There are several options to choose from, look out for the *Red Hat* Cluster Task (CT) one. `*Then hit Add.*`
...
`*Then click on 'Add task'*` and as before `*next to the (+) type 's2i-java'*` When you see the *Red Hat* Cluster Task (CT) one `*select and add the 's2i-java' task*`

`*Click on the newly created 's2i-java' task and enter the following configuration*`:

.s2i-java Cluster Task (CT)
[%header,cols="30%,70%"]
|===
|Parameter 
|Value
...
In the {OPENSHIFT_CONSOLE_URL}[OpenShift Web Console^,role='params-link'], `*click on 'Pipelines' -> 'PL inventory-pipeline' -> 'Actions' -> 'Start'*`
