...
image::openshift-import-from-yaml.png[OpenShift - Import YAML, 600]
...
`*click on the 'Workloads' menu and 'Config Maps' then click on the 'Create Config Map' button*`.
...
[NOTE]
====
It's important that you choose the Red Hat ArgoCD task-sync-and-wait that you created earlier and this is marked with a green (T) 
====


.OpenShift Client Cluster Task Configuration
[%header,cols=3*]
|===
|Section
|Parameter 
...
Once executed, in the {OPENSHIFT_CONSOLE_URL}[OpenShift Web Console^,role='params-link'], `*Click on 'Pipelines' -> 'PL - coolstore-java-pipeline' -> 'Pipeline Runs'*`
...
`*Click on 'Workloads' menu and 'Topology'*` of the {OPENSHIFT_CONSOLE_URL}[OpenShift Web Console^,role='params-link']
