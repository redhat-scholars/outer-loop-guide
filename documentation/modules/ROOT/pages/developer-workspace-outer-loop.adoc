...
`*Select 'my-project{USER_ID}'*` to be taken to the project overview page
...
You should test out the application deployment by `*clicking on the Route icon*` for the NodeJS based web-coolstore component.


== Fix up the Browser URL

If you see the following result in the browser window then its likely your (Chromium) 
browser is upgrading the simple *http://* based URLs to *https* - but the deployed apps don't support that.
